...
import {
  ...
  FormControlLabel, Checkbox, Paper, Alert,
} from '@mui/material';
  ...
  const mutation = useMutation({
    mutationFn: async () => {
      const res = await fetch('https://dummyjson.com/auth/todos/add', {
        ...
        body: JSON.stringify({ todo, completed, userId: Number(userId) }),
      });
      ...
      <Box sx={{ p: 4, maxWidth: 500, margin: 'auto' }}>
        <Paper elevation={3} sx={{ p: 4 }}>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <TextField label="User ID" type="number" fullWidth required value={userId}
          ...
          {status === 'success' && <Alert severity="success" sx={{ mt: 2 }}>Todo added successfully!</Alert>}
          {status === 'error' && <Alert severity="error" sx={{ mt: 2 }}>Failed to add todo.</Alert>}
        </Paper>
      </Box>
